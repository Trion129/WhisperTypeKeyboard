...
class WhisperEngine(
    ...
    fun transcribe(pcmFloats: FloatArray): String {
        ...
        val encResult = encoderSession.run(mapOf(WhisperModelConfig.ENCODER_INPUT to melPadded))
    ...
    private fun decodeLoop(crossK: OnnxTensor, crossV: OnnxTensor): List<Long> {
        ...
        for (step in 0 until MAX_DECODE_LEN) {
            ...
            val tokensTensor = OnnxTensor.createTensor(env, inputTokens.toLongArray(), longArrayOf(1, inputTokens.size.toLong()))
            val offsetTensor = OnnxTensor.createTensor(env, longArrayOf(offset), longArrayOf(1))
    ...
    private fun padMel(mel: OnnxTensor): OnnxTensor {
        ...
        return if (frames >= MAX_FRAMES) {
            ...
            OnnxTensor.createTensor(env, truncated, longArrayOf(1, N_MEL.toLong(), MAX_FRAMES.toLong()))
        } else {
            val padded = FloatArray(N_MEL * MAX_FRAMES)
            System.arraycopy(data, 0, padded, 0, data.size)
            OnnxTensor.createTensor(env, padded, longArrayOf(1, N_MEL.toLong(), MAX_FRAMES.toLong()))
        }
    }

    private fun zeroCache(): OnnxTensor {
        val size = N_DECODER_LAYERS * MAX_DECODE_LEN * D_MODEL
        return OnnxTensor.createTensor(env, FloatArray(size), longArrayOf(N_DECODER_LAYERS.toLong(), 1, MAX_DECODE_LEN.toLong(), D_MODEL.toLong()))
    }
